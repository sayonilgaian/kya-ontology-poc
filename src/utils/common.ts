...
export const token = (v: TokenInput): string => {
	...
	if (
		...
		return `var(--custom-${v.designSystem.toLowerCase()}-${v.theme.toLowerCase()}-${
			v.key
		})`;
	}
...
export const applyStyles = (
	...
	if (applyTheme) {
		...
		el.designSystemService?.applyThemeToElement(
			designSystem,
			theme,
			el,
			variables
...
export const appliers: any = {
	...
	StyleAtom: (e: BaseUIElement, c: any) =>
		applyStyles(e, { [c.role]: c.value, ...c }, c.theming),
		...
		switch (op) {
			...
			case 'Listen':
				...
				if (sig) {
					...
					console.info(
						`Can't listen to ${name}, this action does not exist`
					);
				}
				break;
	...
	KeyboardEventAtom: (e: BaseUIElement, c: any) => {
		...
		if (!e._keyboardService) {
			console.warn(
				'KeyboardEventAtom requires _keyboardService to be initialized'
			);
			return;
		}
		...
		switch (op) {
			case 'Create': {
				if (typeof thirdPartyLibraryName !== 'string') {
					console.warn(
						`Expected class name string in "class", got ${typeof thirdPartyLibraryName}`
					);
					return;
				}
...
export async function executeActionPipeline(
	...
	while (queue.length > 0) {
		...
		if (current === undefined) {
			console.warn(
				'Queue is empty, but there are still unprocessed actions.'
			);
			break;
		}
		const {
			...
			dependencies,
			params,
		} = actionToPerform[current].config;
		...
		try {
			...
			console.error(
				'Error occured, Cannot continue with the current pipeline',
				error
			);
			break;
		}
		...
		if (op === 'Conditional') {
			...
			const loopEndIndex = actionToPerform.findIndex(
				(atom: any, idx: number) =>
					atom.config.op === 'LoopEnd' && idx > (current ?? -1)
			);

			if (loopEndIndex === -1) {
				throw new Error(
					'LoopEnd not found for LoopStart at index ' + current
				);
			}

			const { loopOver, start, end } = actionToPerform[current].config;

			const resolvedDeps = getAtomIdById(
				dependencies,
				actionToPerform,
				results
			);
			...
			if (Array.isArray(data)) {
				...
				endPos =
					endPosRaw !== undefined
						? Math.min(endPosRaw, iterable.length)
						: iterable.length;
			} else if (
				data &&
				typeof data === 'object' &&
			...
			for (let i = 0; i < finalIterable.length; i++) {
				...
				const subActions = structuredClone(
					actionToPerform.slice(current + 1, loopEndIndex)
				);
				...
				for (
					...
					if (Array.isArray(results[j]))
						results[j].push(result[j - (current + 1)]);
				}
