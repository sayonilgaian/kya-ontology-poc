...
export class CytoscapeService implements thirdParty {
    ...
    updateNode(nodeId: string, newData: Partial<NodeData>): NodeSingular | null {
        if (!this.cy) return null;
    ...
    updateEdge(edgeId: string, newData: Partial<EdgeData>): EdgeSingular | null {
        if (!this.cy) return null;
        ...
        if (edge.length > 0) {
            // Update edge data
            Object.keys(newData).forEach(key => {
                if (key !== 'source' && key !== 'target') { // Don't allow changing source/target
